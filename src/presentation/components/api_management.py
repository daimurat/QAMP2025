...
import os
import time
from src.utils.encryption import save_encrypted_key, load_encrypted_key, create_fernet
...
def render_api_management():
    ...
    st.markdown(
        f"OpenAI Key: {'✅ Ready' if openai_loaded else '❌ No Keys'} | Saved: {'🗄️' if openai_file_exists else '—'}"
    )
    st.markdown(
        f"Gemini Key: {'✅ Ready' if gemini_loaded else '❌ No Keys'} | Saved: {'🗄️' if gemini_file_exists else '—'}"
    )
    st.markdown(
        f"OpenRouter Key: {'✅ Ready' if openrouter_loaded else '❌ No Keys'} | Saved: {'🗄️' if openrouter_file_exists else '—'}"
    )
...
def _render_save_button(
    ...
    if (
        (openai_loaded or gemini_loaded or openrouter_loaded)
        and user_password
        and username_display
        and (not openai_file_exists or not gemini_file_exists or not openrouter_file_exists)
    ):
    ...
    if st.button("🔐 Load Saved API Key(s)"):
        if not username or not user_password:
            ...
            if not error:
                ...
                if (
                    (st.session_state.saved_api_key and st.session_state.selected_model in GPT_MODELS)
                    or (st.session_state.saved_api_key_gai and st.session_state.selected_model in GEMINI_MODELS)
                    or (
                        st.session_state.saved_api_key_openrouter
                        and st.session_state.selected_model in OPENROUTER_MODELS
                    )
                ):
